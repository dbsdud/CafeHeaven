...
public class OrderController {
	...
	public String orderDirect(HttpServletRequest request, HttpServletResponse response, Model model,
	...
	public String orderDirectDetail(HttpServletRequest req, HttpServletResponse res, Model model, HttpSession session)
			throws Exception {
		...
		/*
		 * Map<String, TmpDTO> tMap = (Map<String,
		 * TmpDTO>)session.getAttribute("ss_tmpBasket"); if(tMap == null) { tMap = new
		 * HashMap(); } String returnURL = "order/orderDirectDetail"; if(tMap.size()<1)
		 * { returnURL = "user/userAlert"; model.addAttribute("msg", "주문하신 제품이 없습니다.");
	...
	public void orderComplete(HttpServletRequest req, HttpServletResponse res, Model model, HttpSession session)
		...
		if (rep_code.equals("0000")) { // 결제 성공
			...
			log.info(this.getClass() + "ordInfoNo : " + tran_no);
			oDTO.setOrdTotPrice(amt);
			log.info(this.getClass() + "amt : " + amt);
			if (tran_type.equals("PHON")) {
				oDTO.setOrdPayment("p");
				log.info(this.getClass() + "ordPayment : p");
			} else {
			...
			oDTO.setOrdTid(tid);
			/*
			 * 결제 성공시 스팸프 추가 감소,, 나중에 주석 풀어야함 String[] userNoAndStamp =
			 * etc_data1.split(";"); String[] stamp = userNoAndStamp[1].split("-");
			 * Map<String, String> stampMap = new HashMap();
			 * oDTO.setUpdNo(userNoAndStamp[0]); if(stamp[0].equals("dec")) {
			 * oDTO.setStamp(stamp[1]);
			 * oDTO.setOrdTotPrice((Integer.parseInt(amt)+Integer.parseInt(stamp[1])) + "");
			 * oDTO.setRegNo(userNoAndStamp[0]); stampMap.put("dec", stamp[1]); } else {
	...
	public String orderSuccess(HttpServletRequest req, HttpServletResponse res, Model model, HttpSession session)
			throws Exception {
	...
	public @ResponseBody List<TotalOrderDTO> orderInterval(HttpServletRequest req, HttpServletResponse res, Model model)
			throws Exception {
	...
	public @ResponseBody List<TotalOrderDTO> adminTakeOrder(HttpServletRequest req, HttpServletResponse res,
	...
	public @ResponseBody List<TotalOrderDTO> orderCancel(HttpServletRequest req, HttpServletResponse res, Model model)
			throws Exception {
	...
	public @ResponseBody List<TotalOrderInfoDTO> adminOrderRemainTime(HttpServletRequest req, HttpServletResponse resp,
			Model model) throws Exception {
	...
	// 주문 테스트
	/*
	 * @RequestMapping(value="orderProcTest", method=RequestMethod.POST) public
	 * String orderProcTest(HttpServletRequest req, HttpServletResponse res, Model
	 * model, HttpSession session) throws Exception{ log.info(this.getClass() +
	 * " orderProcTest Start!"); String userNo =
	 * CmmUtil.nvl(req.getParameter("userNo")); log.info(this.getClass() +
	 * " userNo : " + userNo); String ordTotPrice =
	 * CmmUtil.nvl(req.getParameter("ordTotPrice")); log.info(this.getClass() +
	 * " ordTotPrice : " + ordTotPrice); String ordPayment =
	 * CmmUtil.nvl(req.getParameter("ordPayment")); log.info(this.getClass() +
	 * " ordPayment : " + ordPayment); String ordDtDate =
	 * CmmUtil.nvl(req.getParameter("ordDtDate")); log.info(this.getClass() +
	 * " ordDtDate : " + ordDtDate); String ordStat =
	 * CmmUtil.nvl(req.getParameter("ordStat")); log.info(this.getClass() +
	 * " ordStat : " + ordStat);
	 * 
	 * 
	 * 
	 * log.info(this.getClass() + " orderProcTest End!"); return null; }
	 */
