...
<body>
	...
	<div class="container">
		<div class="page-header">
			<h1>
				게시판 <small>CAFE HEAVEN</small>
			</h1>
		</div>
		<div class="contents">
			<!--  탭  구성 상단 -->
			<ul class="nav nav-tabs">
				<li class="active" style="width: 33%;"><a data-toggle="tab"
					href="#menu1" aria-expanded="true">공지사항</a></li>
				<!-- class= active 활성화된탭 -->
				<li style="width: 34%;" class=""><a data-toggle="tab"
					href="#menu2" aria-expanded="false">Q&amp;A</a></li>
				<!-- <li style="width: 33%;" class=""><a data-toggle="tab" href="#menu3" id="reviewListView" aria-expanded="false">리뷰</a></li> -->
				<li style="width: 33%;" class=""><a data-toggle="tab"
					href="#menu3" id="reviewListView" aria-expanded="false">리뷰</a></li>
			</ul>
			<!--  tab클릭 내용 -->
			<div class="tab-content">
				...
				<div id="menu1" class="tab-pane in active">
					...
					<div class="hd-list ">
						...
						<div style="height: 50px">
							<p>
								<a
									href="/notice/noticeDetail.do?regNo=<%=nList.get(i).getRegNo()%>&ntNo=<%=nList.get(i).getNtNo() %>"><%=nList.get(i).getNtTitle() %></a>
							</p>
						</div>
						...
						<div style="height: 50px">
							<p><%=DateUtil.DateFormat(nList.get(i).getRegDate()) %></p>
						</div>
						<%} else { %>
						<div style="height: 50px">
							<p><%=DateUtil.DateFormat(nList.get(i).getUpdDate()) %></p>
						</div>
						<%} %>
						<!--  조회수 -->
						<div style="height: 50px">
							<p><%=nList.get(i).getNtCount() %></p>
						</div>
						<!--  이건 나중에 카운트 해서넣기 -->
					</div>
					<%} %>
					<%} %>
					<% if(userNo.equals("1")) { %>
					<div id="addNotice" style="text-align: center;">
						<a href="/notice/noticeReg.do" style="width: 50%;"
							class="btn btn-primary">글쓰기</a>
					</div>
					<% } %>
					<br>
					<div class="boardSearch" style="width: 100%;">
				...
				<div id="menu2" class="tab-pane">
					...
					<div class="hd-list-two">
						<div style="height: 50px">
							...
							<p>
								<a
									href="/notice/questionDetail.do?regNo=<%=nList.get(i).getRegNo()%>&ntNo=<%=nList.get(i).getNtNo() %>"><%=nList.get(i).getNtTitle() %></a>
							</p>
						</div>
					...
					<%} %>
					<% if(!userNo.equals("")) { %>
					<div id="adQa" style="text-align: center;">
						<a href="/notice/questionReg.do" style="width: 50%;"
							class="btn btn-primary">글쓰기</a>
					</div>
					<%} %>
					<br> <a href="#" id="load-two" style="width: 100%"
						class="btn btn-primary">더보기</a> <br /> <br />
				</div>
				...
				<div id="menu3" class="tab-pane">
					...
					<%if (userNo !="") {%>
					<!-- 보내기  폼-->
					<form id="ajaxform" action="/notice/cafeReviewRegProc.do"
						method="post">
						<!-- 회원이 구매한 메뉴 보여주기 -->
					...
					$(function() {
					   $('#example').barrating({
						...
						<div style="width: 30%; float: left">
							<input type="text" id="cafeReview" name="cafeReview"
								class="form-control" placeholder="리뷰를 등록해주세요." />
						</div>
						<div>
